...
const HomePage = ({fetchShoppinglist, fetchItems, expiringItems, setExpringItems}) => {
    ...
    const updateItem = async (item, updatetext, updatetype) => {
        const itemIndex = expiringItems.findIndex(item => item.id === item.id);
        ...
        const updatedItem = { ...expiringItems[itemIndex] };
        ...
        // Create a copy of the expiringItems array
        const updatedItems = [...expiringItems];
    
        // Replace the old item with the updated item in the copy of the expiringItems array
        updatedItems[itemIndex] = updatedItem;
    
        // Update the expiringItems state with the new array
        setExpringItems(updatedItems);
    ...
    return (
        <View style={[styles.container, { backgroundColor: colorScheme === 'light' ? '#f5f5f5' : '#202225', color: colorScheme === 'light' ? '#202225' : '#f5f5f5' }]}>
            ...
            <Text style={[styles.header, {color: colorScheme === 'light' ? '#202225' : '#f5f5f5' }]}>Expiring Items -&gt;</Text>
            ...
            />) : (
                <Text style={{color: colorScheme === 'light' ? '#202225' : '#f5f5f5'}}>No items</Text>
            )}
